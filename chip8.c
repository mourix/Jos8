	...
	switch(opcode & 0xF000)
	{
		case 0x0000:
			switch(opcode & 0x000F)
			{
				...
				case 0x0000:
					if(debug) printf("0x%X Opcode 0x%X: 00E0 disp_clear\n", pc, opcode);
					memset(screen, 0, sizeof(screen[0][0])*64*32);
					pc += 2;
				...
				case 0x000E:
					if(debug) printf("0x%X Opcode 0x%X: 00EE return\n", pc, opcode);
					sp--;
					pc = stack[sp];
					break;
					
				default:
					printf ("0x%X Unknown opcode: 0x%X\n", pc, opcode);
			}
			break;
		
		// 1NNN 	Flow 	goto NNN; 	Jumps to address NNN.
		case 0x1000:
			if(debug) printf("0x%X Opcode 0x%X: 1NNN goto NNN\n", pc, opcode);
			pc = opcode & 0x0FFF;
			break;
			
		// 2NNN 	Flow 	*(0xNNN)() 	Calls subroutine at NNN.	
		case 0x2000:
			if(debug) printf("0x%X Opcode 0x%X: 2NNN Call subroutine NNN\n", pc, opcode);
			stack[sp] = pc + 2;
			sp++;
			pc = opcode & 0x0FFF;
			break;
		...
		case 0x3000:
			if(debug) printf("0x%X Opcode 0x%X: 3XNN skip if(Vx==NN)\n", pc, opcode);
		...
		case 0x4000:
			if(debug) printf("0x%X Opcode 0x%X: 4XNN skip if(Vx!=NN)\n", pc, opcode);
			if(V[(opcode & 0x0F00) >> 8] != (opcode & 0x00FF)) pc += 4;
			else pc += 2;		
			break;
		...
		case 0x5000:
			if(debug) printf("0x%X Opcode 0x%X: 5XY0 skip if(Vx==Vy)\n", pc, opcode);
		...
		case 0x6000:
			if(debug) printf("0x%X Opcode 0x%X: 6XNN Vx = NN \n", pc, opcode);
		...
		case 0x7000:
			if(debug) printf("0x%X Opcode 0x%X: 7XNN Vx += NN\n", pc, opcode);
			V[(opcode & 0x0F00) >> 8] += opcode & 0x00FF;
			pc += 2;
			break;
		...
		case 0x8000:
			switch(opcode & 0x000F)
			{
				...
				case 0x0000:
					if(debug) printf("0x%X Opcode 0x%X: 8XY0 Vx=Vy\n", pc, opcode);
				...
				case 0x0001:
					if(debug) printf("0x%X Opcode 0x%X: 8XY1 Vx|Vy\n", pc, opcode);
				...
				case 0x0002:
					if(debug) printf("0x%X Opcode 0x%X: 8XY2 Vx=Vx&Vy\n", pc, opcode);
					V[(opcode & 0x0F00) >> 8] = V[(opcode & 0x0F00) >> 8] & V[(opcode & 0x00F0) >> 4];
					pc += 2;
					break;
				...
				case 0x0003:
					if(debug) printf("0x%X Opcode 0x%X: 8XY3 Vx=Vx^Vy\n", pc, opcode);
					V[(opcode & 0x0F00) >> 8] = V[(opcode & 0x0F00) >> 8] ^ V[(opcode & 0x00F0) >> 4];
					pc += 2;
					break;
				...
				case 0x0004:
					if(debug) printf("0x%X Opcode 0x%X: 8XY4 Vx += Vy\n", pc, opcode);
				...
				case 0x0005:
					if(debug) printf("0x%X Opcode 0x%X: 8XY5 Vx -= Vy\n", pc, opcode);
				...
				case 0x0006:
					if(debug) printf("0x%X Opcode 0x%X: 8XY6 Vx=Vy=Vy>>1\n", pc, opcode);
					if(cowgod)
					{
				...
				// VF is set to 0 when there's a borrow, and 1 when there isn't.
				case 0x0007:
					if(debug) printf("0x%X Opcode 0x%X: 8XY7 Vx=Vy-Vx\n", pc, opcode);
					if(V[(opcode & 0x0F00) >> 8] > V[(opcode & 0x00F0) >> 4]) V[0xF] = 0;
					else V[0xF] = 1;
				...
				case 0x000E:
					if(debug) printf("0x%X Opcode 0x%X: 8XYE Vx=Vy=Vy<<1\n", pc, opcode);
					if(cowgod)
					{
		...
		case 0x9000:
			if(debug) printf("0x%X Opcode 0x%X: 9XY0 skip if(Vx!=Vy)\n", pc, opcode);
			if(V[(opcode & 0x0F00) >> 8] != V[(opcode & 0x00F0) >> 4]) pc += 4;
			else pc += 2;
			break;
		...
		case 0xA000:
			if(debug) printf("0x%X Opcode 0x%X: ANNN I = NNN\n", pc, opcode);
			I = opcode & 0x0FFF;
			pc += 2;
			break;
		...
		case 0xB000:
			if(debug) printf("0x%X Opcode 0x%X: BNNN PC=V0+NNN\n", pc, opcode);
		...
		case 0xC000:
			if(debug) printf("0x%X Opcode 0x%X: CXNN Vx=rand()&NN\n", pc, opcode);
		...
		case 0xD000:;
			if(debug) printf("0x%X Opcode 0x%X: DXYN draw(Vx,Vy,N)\n", pc, opcode);
		...
		case 0xE000:
			switch(opcode & 0x000F)
			{
				...
				case 0x000E:
					if(debug) printf("0x%X Opcode 0x%X: EX9E if(key()==Vx)\n", pc, opcode);
				...
				case 0x0001:
					if(debug) printf("0x%X Opcode 0x%X: EXA1 if(key()!=Vx)\n", pc, opcode);
					if(!key[V[(opcode & 0x0F00) >> 8]]) pc += 4;
					else pc += 2;
					break;
		...
		case 0xF000:
			switch(opcode & 0x00FF)
			{
				...
				case 0x0007:
					if(debug) printf("0x%X Opcode 0x%X: FX07 Vx = get_delay()\n", pc, opcode);
				...
				case 0x000A:
					if(debug) printf("0x%X Opcode 0x%X: FX0A\n", pc, opcode);
				...
				case 0x0015:
					if(debug) printf("0x%X Opcode 0x%X: FX15 delay_timer(Vx)\n", pc, opcode);
				...
				case 0x0018:
					if(debug) printf("0x%X Opcode 0x%X: FX18 sound_timer(Vx)\n", pc, opcode);
				...
				case 0x001E:
					if(debug) printf("0x%X Opcode 0x%X: FX1E I +=Vx\n", pc, opcode);
					I += V[(opcode & 0x0F00) >> 8];
					pc += 2;
					break;	
				...
				case 0x0029:
					if(debug) printf("0x%X Opcode 0x%X: FX29 I=sprite_addr[Vx] \n", pc, opcode);
				...
				case 0x0033:
					if(debug) printf("0x%X Opcode 0x%X: FX33 set_BCD(Vx);\n", pc, opcode);
				...
				case 0x0055:
					if(debug) printf("0x%X Opcode 0x%X: FX55 reg_dump(Vx,&I)\n", pc, opcode);
				...
				// I is increased by 1 for each value written.
				case 0x0065:
					if(debug) printf("0x%X Opcode 0x%X: FX65 reg_load(Vx,&I)\n", pc, opcode);
					for(int i = 0; i <= ((opcode & 0x0F00) >> 8); i++)
					{
						V[i] = memory[I];
						I++;
